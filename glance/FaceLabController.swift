...
final class FaceLabController {
    ...
    func recognize() {
        ...
        let liveGeometry = FaceGeometryTemplate.extract(from: result)
        let scored = pipeline.score(result.embedding, against: store.identities, liveGeometry: liveGeometry)
