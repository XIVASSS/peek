...
struct FaceSample: Codable, Equatable {
    ...
    let quality: Float?
    /// Compact 3D-proxy landmark template for this pose. Optional for legacy enrollments.
    ...
    init(
        ...
        capturedAt: Date,
        quality: Float?,
        geometry: FaceGeometryDescriptor? = nil
    ) {
    ...
    init(from decoder: Decoder) throws {
        ...
        quality = try container.decodeIfPresent(Float.self, forKey: .quality)
        geometry = try container.decodeIfPresent(FaceGeometryDescriptor.self, forKey: .geometry)
    }
}
...
extension FaceSample {
    ...
    var qualityTier: QualityTier {
        ...
        // Mac webcam Vision scores often land 0.25–0.45; the old 0.4 "poor" floor
        // made healthy enrollments look like "3/18 low" in Settings.
        if quality < 0.22 { return .poor }
...
struct FaceIdentity: Codable, Identifiable, Equatable {
    ...
    var isEnabled: Bool
    /// Aggregated multi-pose 3D-proxy profile (shape templates + nose-parallax signature).
    var geometryProfile: FaceGeometryProfile?
    ...
    init(
        ...
        createdAt: Date,
        isEnabled: Bool = true,
        geometryProfile: FaceGeometryProfile? = nil
    ) {
        self.id = id
        self.name = name
        ...
        self.geometryProfile = geometryProfile ?? Self.makeGeometryProfile(from: samples)
    }

    /// Hand-written so `isEnabled` / `geometryProfile` default safely when absent — a synthesized decoder would throw
    /// on a missing key and, since the whole store decodes as one array, lose every identity in it.
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        ...
        if let stored = try container.decodeIfPresent(FaceGeometryProfile.self, forKey: .geometryProfile) {
            geometryProfile = stored
        } else {
            geometryProfile = Self.makeGeometryProfile(from: samples)
        }
    }
    ...
    nonisolated static func makeGeometryProfile(from samples: [FaceSample]) -> FaceGeometryProfile? {
        let descriptors = samples.compactMap(\.geometry)
        guard !descriptors.isEmpty else { return nil }
        return FaceGeometryTemplate.profile(from: descriptors)
    }
}
...
final class FaceEnrollmentStore {
    ...
    func addSample(name: String, embedding: [Float], embedder: FaceEmbedder, pose: String? = nil, quality: Float? = nil) throws -> Bool {
        ...
        let sample = FaceSample(embedding: embedding, pose: pose, capturedAt: Date(), quality: quality, geometry: nil)
        ...
        if let index = identities.firstIndex(where: { $0.name == trimmed }) {
            ...
            identities[index].geometryProfile = FaceIdentity.makeGeometryProfile(from: identities[index].samples)
        } else {
    ...
    func commitEnrollment(
        ...
        let geometryProfile = FaceIdentity.makeGeometryProfile(from: samples)
        if let existingID, let index = updated.firstIndex(where: { $0.id == existingID }) {
            ...
            updated[index].geometryProfile = geometryProfile
