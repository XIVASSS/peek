...
struct YourFaceSettingsPage: View {
    ...
    private var unreadableState: some View {
        VStack(spacing: SettingsMetrics.emptyStateSpacing) {
            ...
            SettingsCaption(text: "Nothing has been deleted, and Peek will not overwrite it — enrolling is blocked until this resolves. Quit and reopen Peek to retry. If it keeps failing, the session key no longer matches this data: remove the stored password on the Password tab to clear both, then set up again.")
                .multilineTextAlignment(.center)
        }
    ...
    private var faceEncryptedCard: some View {
        SettingsGroup {
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("Enroll separate identities to use Peek with multiple people, accessories (ex. glasses), facial expressions, or new lighting environments. This improves recognition quality.")
                    .font(.system(size: 12))
...
private struct IdentityCard: View {
    ...
    private var qualityCaption: String {
        ...
        let goodCount = identity.samples.filter { $0.qualityTier == .good }.count
        let fairCount = identity.samples.filter { $0.qualityTier == .fair }.count
        if poorCount == 0 {
            return "Capture quality • \(goodCount) good\(fairCount > 0 ? ", \(fairCount) fair" : "")"
        }
        return "Capture quality • \(goodCount) good · \(poorCount) weak"
    }
